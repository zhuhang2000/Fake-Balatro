...
((root) => {
  const Snd = {
    ...
    tone(o = {}) {
      if (!this.ctx) return;
      const { f = 440, f2 = null, type = 'square', dur = 0.1, vol = 0.2, at = 0 } = o;
      const t0 = this.t() + at;
    ...
    noise(o = {}) {
      if (!this.ctx) return;
      const { dur = 0.1, vol = 0.2, at = 0, ftype = 'bandpass', f = 2000, f2 = null, q = 1 } = o;
      const t0 = this.t() + at;
      ...
      if (f2) fl.frequency.exponentialRampToValueAtTime(Math.max(f2, 1), t0 + dur);
      const g = this.ctx.createGain();
      g.gain.setValueAtTime(0.0001, t0);
